...
./filePartition5Jun2014 $5 $7 $8 ${12}
...
lastWindowFile=$5_lastWindowData_partitions=$7_windowSize=$8.txt
...
#changing the timestamps in the output file
./timestamp $inputFileOne $outputFileOne ${12}
while [ $iterator -le $8 ]
do
...
./output $m $M $Epsilon $Epsilon1 $MINPOINTS $UNDEFINED 0 $iterator $3 $4 $lastWindowFile OutputResults/output_batchwise_incremetal_optics_$lastWindowFile\_partition=1\_windowSize=1\_m=$m\_M=$M\_EPS=$Epsilon\_MINPOINTS=$MINPOINTS.txt $9 random.txt $finalInputFile
fi
rm $5"_partition_"$7*
rm $lastWindowFile
#rm rdUndef*
