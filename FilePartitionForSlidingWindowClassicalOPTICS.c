...
int main( int argc, char** argv)
{
	...
	{
		sum=0;
		sprintf(partitionedFilename,"%s_partitions_%d_windowSize_%d_window_%d_%d",argv[1],noOfPartitions,windowSize,1,i);
		fpartition=fopen(partitionedFilename,"w");
		for(j=1;j<=i;j++)
		{
	...
	for(i=2;i<=noOfPartitions- windowSize +1;i++)
	{
		sum=0;
		sprintf(partitionedFilename,"%s_partitions_%d_windowSize_%d_window_%d_%d",argv[1],noOfPartitions,windowSize,i,i+windowSize-1);
		fpartition=fopen(partitionedFilename,"w");
		for(j=i;j<i+windowSize;j++)
		{
			sprintf(partitionedFilename,"%s_partition_%d_%d",argv[1],noOfPartitions,j);
			fTemp[j-i]=fopen(partitionedFilename,"r");
			fscanf(fTemp[j-i],"%d\n%d\n",temp+j-i,&noOfDim);
			sum+=temp[j-i];
		}
		fprintf(fpartition, "%d\n%d",sum,noOfDim );
		for(j=i;j<i+windowSize;j++)
		{
			for(k=0;k<temp[j-i];k++)
			{
				fprintf(fpartition, "\n" );
				for(m=0;m<noOfDim;m++)
				{
					fscanf(fTemp[j-i],"%lf",&tempRead);
					fprintf(fpartition, "%lf ",tempRead );
				}
			}
		}
